...
def predict(dn=None, dist_group=None, hash_=None, yard_zone=None,
    ...
    # If high % Pass, also show most likely pass direction / side
    if result["pass_pct"] >= THRESHOLD_STRONG:
        pass_sub = sub[sub["PLAY TYPE"] == "Pass"]
        if len(pass_sub) >= 3:
            pdir = pct_series(pass_sub["PLAY DIR"])
            if pdir:
                result["pass_dir"] = pdir[0]  # (dir, pct, count)
                # Also add as trend if strong enough
                if pdir[0][1] >= 60:
                    result["trends"].append(
                        (f"Pass goes {pdir[0][0]}", pdir[0][1], pdir[0][2])
                    )
...
if predict_btn:
    ...
    # Strong trend
        ...
        if result.get("pass_dir"):
            d, p, c = result["pass_dir"]
            st.markdown(f'<p class="detail">Pass side tendency: {d} ({p}%)</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
