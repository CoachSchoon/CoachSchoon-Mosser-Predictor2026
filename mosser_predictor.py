...
def filter_plays(dn=None, dist_group=None, hash_=None, yard_zone=None,
    ...
    # Start from full data, optionally restricted to active formations
    base = DF
...
with col2:
    ...
    all_forms = sorted([f for f in DF["OFF FORM"].dropna().unique().tolist() if f])

# Active formations for this game (exclude unused ones from all predictions)
st.markdown("**Formations in use this game**")
st.caption("Uncheck formations they are NOT using. All predictions only use the checked ones.")
if "active_forms" not in st.session_state:
    st.session_state.active_forms = all_forms[:]

active_forms = st.multiselect(
    "Active formations",
    options=all_forms,
    default=[f for f in st.session_state.active_forms if f in all_forms],
    label_visibility="collapsed",
)
st.session_state.active_forms = active_forms

if active_forms:
    n_active = int(DF["OFF FORM"].isin([str(f).upper() for f in active_forms]).sum())
    st.caption(f"{len(active_forms)} formations selected • {n_active} plays in pool")
    form_choices = active_forms  # this-play dropdown only shows active ones
else:
    st.warning("No formations selected — predictions will use all data.")
    form_choices = all_forms
