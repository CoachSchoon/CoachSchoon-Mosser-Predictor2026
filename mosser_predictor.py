...
def _build_trends(sub):
    ...
    # Top run plays (from run subset)
    run_sub = sub[sub["PLAY TYPE"] == "Run"]
    if len(run_sub) > 0:
        top_runs = pct_series(run_sub["OFF PLAY"])[:2]
...
for btn_label, zone_value in FIELD_BUTTONS:
    # Highlight selected
    display = f"▶ {btn_label} ◀" if st.session_state.selected_zone == zone_value else btn_label
    if st.button(display, key=f"field_{btn_label}", use_container_width=True):
...
if predict_btn:
    ...
    # Strong trend
        ...
        # Run / Pass split
        st.markdown(
            f'<p class="pred">Run {result["run_pct"]}% &nbsp;&nbsp;|&nbsp;&nbsp; Pass {result["pass_pct"]}%</p>',
            unsafe_allow_html=True
        )
        ...
        if top_runs:
            ...
            for label, pct, cnt in top_runs:
                st.markdown(
                    f'<p class="detail">{pct}%  {label}  ({cnt} plays)</p>',
                    unsafe_allow_html=True
                )
        else:
        ...
        # Any other strong trends (motion, specific play, etc.)
        extra = [t for t in result["trends"]
                 if t[0] not in ("Run", "Pass") and not str(t[0]).startswith("Pass goes")]
        if extra:
            st.markdown("**Other**")
            for label, pct, cnt in extra:
                st.markdown(
                    f'<p class="detail">{pct}%  {label}  ({cnt} of {result["sample_size"]})</p>',
                    unsafe_allow_html=True
                )
